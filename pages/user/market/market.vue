<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true"><block slot="content">营销活动</block></cu-custom>
		<view class="uni-padding-wrap uni-common-mt">
			<uni-segmented-control style="width: 100%;" :current="current" :values="items" :style-type="styleType" :active-color="activeColor" @clickItem="onClickItem" />
		</view>
		<view class="content" @click="select">
			<view v-if="current === 0"><text class="content-text">+新增卡卷活动</text></view>
			<view v-if="current === 1"><text class="content-text">+新增折扣活动</text></view>
			<view v-if="current === 2"><text class="content-text">+新增满减活动</text></view>
		</view>
	...
	export default {
		...
		methods: {
			...
			select(){
				if(this.current == 0){
					uni.navigateTo({
						url: '/pages/user/market/cardroll',
					});
				}
				if(this.current == 1){
					uni.navigateTo({
